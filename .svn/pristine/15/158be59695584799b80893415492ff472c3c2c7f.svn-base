 ...
 * Author: luowei
 * Date: 2019/06/03
 * based on jquery1.11.1
 */
$(function () {
    var headTableId = $($("table")[0]).attr('id');
    dataTableId =$($("table")[1]).attr('id');
    /**
     * 存储选择器(指定表格的表头)
    ...
    window.onbeforeunload = function () {
        var data = {"url": window.location.pathname, "filterMap": JSON.stringify(filterMap)};
        $.ajax({
            url: "saveFilterData.do",
            type: "post",
...
function addSearchFrame(index) {
    ...
    /*var domList = $("#searchTableData3>tbody>tr").find("td:eq(" + index + ")").sort(function (dom1, dom2) {
        return $(dom1).text() > $(dom2).text() ? 1 : -1;
    });*/
    /**
     * 记录当前td的数据
    ...
    var innerHTML = "<div id='searchFilterDiv'>" +
        "<input type='search' value='' size='35' placeholder='搜索'>" +
        "<span style='cursor:pointer'><i class='search_right' onclick='searchFilterByInput()'></i></span>" +
        "</div>" +
        "<div class='filtrate'>" +
        "<ul>" +
        ...
        "</div>" +
        "<div class='searchBtn'>" +
        "<input class='btn' type='submit' name='submit' value='確 定' onclick='selectData(" + index + ")'>" +
...
function tableSort(index) {
    ...
    domList.sort(function (a, b) {
        if (!desc) {
            return $(a).find("td:eq(" + index + ")").text() > $(b).find("td:eq(" + index + ")").text() ? 1 : -1
        } else {
            return $(a).find("td:eq(" + index + ")").text() < $(b).find("td:eq(" + index + ")").text() ? 1 : -1
        }
    });
 ...
 * @param filterMap
 */
function filterShowBack(filterMap) {
    var preKey = "";
        ...
        if (filterMap.hasOwnProperty(key)) {
            ...
            if (preKey === "" || preKey === key) {
                filterMap[key].forEach(function (value) {
                    var checkedValue = value;
                    $("#" + dataTableId + ">tbody>tr").each(function () {
                        var thisValue = $(this).find("td:eq(" + key + ")").text().replace(/\s*/g, "");
                        if (isHide) {
                            $(this).hide();
                        }
                        if (checkedValue === thisValue) {
                ...
                filterMap[key].forEach(function (value) {
                    var checkedValue = value;
                    domGroupList[domGroupList.length - 1].forEach(function (value) {
                        var thisValue = value.find("td:eq(" + key + ")").text().replace(/\s*/g, "");
                        if (isHide) {
                            value.hide();
                        }
                        if (checkedValue === thisValue) {
            ...
            tableCheckedIdList.push(Number(key));
            domGroupList.push(toShowDomList);
            preKey = key;
        }
    }
